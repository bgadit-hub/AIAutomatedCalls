...
@media(max-width:767px){
  ...
  .aac-topbar{padding:0 16px!important;}
  .aac-content{padding-bottom:80px!important;}
...
label{font-size:12px;color:var(--t2);font-weight:500;display:block;margin-bottom:4px;}
table{width:100%;border-collapse:collapse;}
th{font-size:11px;font-weight:600;color:var(--t3);text-transform:uppercase;letter-spacing:.06em;padding:10px 14px;text-align:left;border-bottom:1px solid var(--border);}
td{font-size:13px;color:var(--t2);padding:12px 14px;border-bottom:1px solid var(--border);vertical-align:middle;}
...
const Empty=({msg})=>(<div style={{padding:'40px 0',textAlign:'center',color:'var(--t3)',fontSize:13}}>{msg}</div>);
...
const ClientAvailability=()=>{
  ...
  const bookingUrl=clientId?`https://aiautomatedcalls.com/book?client_id=${clientId}`:`https://aiautomatedcalls.com/book?client_id=[your-id]`;
  useEffect(()=>{
    const load=async()=>{
      setLoading(true);
      ...
      const {data:rows}=await supabase.from('availability').select('*').eq('client_id',client.id);
      if(rows?.length){const m={...DEFAULT};rows.forEach(r=>{m[r.day_of_week]={active:r.is_active,start:r.start_time.slice(0,5),end:r.end_time.slice(0,5)};});setAvail(m);}
      const {data:s}=await supabase.from('booking_settings').select('*').eq('client_id',client.id).single();
      if(s){setSettings({duration:s.appointment_duration||30,buffer:s.buffer_time||15,min_notice:s.min_notice_hours||2,advance_days:s.advance_booking_days||30,appt_types:Array.isArray(s.appointment_types)?s.appointment_types.join(','):'New Patient,Follow-up,Cleaning'});}
      setLoading(false);
    };
    load();
  },[]);
  const setDay=(d,f,v)=>setAvail(a=>({...a,[d]:{...a[d],[f]:v}}));
  const save=async()=>{
    setSaveErr('');
    if(!clientId){setSaveErr('No client record linked to your account.');return;}
    const rows=Object.entries(avail).map(([d,v])=>({client_id:clientId,day_of_week:parseInt(d),start_time:v.start+':00',end_time:v.end+':00',is_active:v.active}));
    const {error:ae}=await supabase.from('availability').upsert(rows,{onConflict:'client_id,day_of_week'});
    if(ae){setSaveErr('Failed to save: '+ae.message);return;}
    const {error:se}=await supabase.from('booking_settings').upsert({client_id:clientId,appointment_duration:settings.duration,buffer_time:settings.buffer,min_notice_hours:settings.min_notice,advance_booking_days:settings.advance_days,appointment_types:settings.appt_types.split(',').map(t=>t.trim()).filter(Boolean)},{onConflict:'client_id'});
    if(se){setSaveErr('Failed to save settings: '+se.message);return;}
    setSaved(true);setTimeout(()=>setSaved(false),2500);
  };
  const copyLink=()=>{navigator.clipboard.writeText(bookingUrl);setLinkCopied(true);setTimeout(()=>setLinkCopied(false),2000);};
  if(loading)return(<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:300,gap:10,color:'var(--t3)'}}><Loader size={18} className="aac-spin"/>Loading…</div>);
  return(
    <div className="fade-in">
      <SH title="Availability" sub="Set your hours · Patients book via your booking link" action={<button className="btn btn-primary" onClick={save}>{saved?<><Check size={13}/>Saved!</>:<><Check size={13}/>Save Changes</>}</button>}/>
      ...
      <div className="card" style={{marginBottom:16,background:"linear-gradient(135deg,rgba(31,168,160,.05),rgba(61,123,217,.03))",border:"1px solid rgba(31,168,160,.15)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"var(--accent)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:4}}>Your Patient Booking Link</div><div style={{fontSize:12,color:"var(--t2)",fontFamily:"monospace",wordBreak:"break-all",marginBottom:4}}>{bookingUrl}</div><div style={{fontSize:11,color:"var(--t3)"}}>Share with patients — they'll see your available slots and book directly.</div></div>
          <div style={{display:"flex",gap:8,flexShrink:0}}><button className="btn btn-ghost" onClick={copyLink} style={{fontSize:12}}>{linkCopied?<><Check size={12}/>Copied!</>:<><Copy size={12}/>Copy link</>}</button>{clientId&&<a href={bookingUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{fontSize:12,textDecoration:"none"}}><Link size={12}/>Preview</a>}</div>
        </div>
      </div>
      <div className="card" style={{marginBottom:16}}>
        <div style={{fontSize:14,fontWeight:600,marginBottom:16}}>Weekly Hours</div>
        {DAYS.map(({d,label},i)=>(<div key={d} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 0",borderBottom:i<6?"1px solid var(--border)":"none",flexWrap:"wrap"}}><Toggle checked={avail[d].active} onChange={v=>setDay(d,"active",v)}/><span style={{fontSize:13,fontWeight:500,color:avail[d].active?"var(--t1)":"var(--t3)",width:96,flexShrink:0}}>{label}</span>{avail[d].active?(<div style={{display:"flex",alignItems:"center",gap:10,flex:1,flexWrap:"wrap"}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:11,color:"var(--t3)"}}>From</span><input type="time" value={avail[d].start} onChange={e=>setDay(d,"start",e.target.value)} style={{width:110,padding:"5px 8px",fontSize:13}}/></div><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:11,color:"var(--t3)"}}>To</span><input type="time" value={avail[d].end} onChange={e=>setDay(d,"end",e.target.value)} style={{width:110,padding:"5px 8px",fontSize:13}}/></div><span style={{fontSize:12,color:"var(--t3)"}}>{(()=>{const[sh,sm]=avail[d].start.split(":").map(Number);const[eh,em]=avail[d].end.split(":").map(Number);const mins=(eh*60+em)-(sh*60+sm);const slots=Math.max(0,Math.floor(mins/(settings.duration+settings.buffer)));return slots>0?`~${slots} slots`:null;})()}</span></div>):(<span style={{fontSize:13,color:"var(--t3)"}}>Closed</span>)}</div>))}
      </div>
      <div className="card"><div style={{fontSize:14,fontWeight:600,marginBottom:16}}>Booking Settings</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:16}}><div><label>Appointment duration</label><select value={settings.duration} onChange={e=>setSettings(s=>({...s,duration:+e.target.value}))}>{[15,20,30,45,60,90].map(v=><option key={v} value={v}>{v} min</option>)}</select></div><div><label>Buffer between appointments</label><select value={settings.buffer} onChange={e=>setSettings(s=>({...s,buffer:+e.target.value}))}>{[0,5,10,15,20,30].map(v=><option key={v} value={v}>{v} min</option>)}</select></div><div><label>Minimum advance notice</label><select value={settings.min_notice} onChange={e=>setSettings(s=>({...s,min_notice:+e.target.value}))}>{[1,2,4,8,24,48].map(v=><option key={v} value={v}>{v}h</option>)}</select></div><div><label>Book up to (days in advance)</label><select value={settings.advance_days} onChange={e=>setSettings(s=>({...s,advance_days:+e.target.value}))}>{[7,14,30,60,90].map(v=><option key={v} value={v}>{v} days</option>)}</select></div><div style={{gridColumn:"1/-1"}}><label>Appointment types (comma-separated)</label><input value={settings.appt_types} onChange={e=>setSettings(s=>({...s,appt_types:e.target.value}))}/><div style={{fontSize:11,color:"var(--t3)",marginTop:4}}>Patients choose from these on your booking page.</div></div></div></div>
    </div>
  );
};
  ...
  return(
    <div style={{minHeight:"100vh",background:"#FFFFFF",fontFamily:"var(--font)",color:"var(--t1)"}}>
      ...
      <section style={{padding:"88px 5% 72px",textAlign:"center",maxWidth:860,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(31,168,160,0.08)",border:"1px solid rgba(31,168,160,0.2)",borderRadius:100,padding:"5px 14px",marginBottom:28}}><span style={{width:7,height:7,borderRadius:"50%",background:"var(--accent)",display:"inline-block"}}/><span style={{fontSize:12,fontWeight:600,color:"var(--accent)",letterSpacing:".04em"}}>LIVE IN 48 HOURS · NO CONTRACTS</span></div>
        ...
        {submitStatus==='success'?(
          <div style={{maxWidth:500,margin:'0 auto',background:'rgba(5,150,105,0.08)',border:'1px solid rgba(5,150,105,0.2)',borderRadius:12,padding:'28px',textAlign:'center'}}><div style={{fontSize:36,marginBottom:10}}>✅</div><div style={{fontSize:18,fontWeight:700,color:'var(--success)',marginBottom:8}}>You're all set!</div><div style={{fontSize:15,color:'var(--t2)',lineHeight:1.65}}>Expect a call within the next 60 seconds.</div></div>
        ):(
          <div style={{maxWidth:500,margin:'0 auto'}}>
            <div style={{display:'grid',gap:10,marginBottom:12}}>
              ...
              <select value={form.business_type} onChange={e=>setForm(f=>({...f,business_type:e.target.value}))} style={{padding:'13px 14px',fontSize:14,borderRadius:8,border:'1px solid var(--border2)',fontFamily:'var(--font)',background:'#fff',color:form.business_type?'var(--t1)':'#9CA3AF'}}><option value="">What type of business? (optional)</option><option>Dental practice</option><option>Real estate agency</option><option>HVAC / plumbing</option><option>Med spa</option><option>Law firm</option><option>Chiropractic</option><option>Other</option></select>
              {errMsg&&<div style={{fontSize:13,color:'var(--danger)',textAlign:'left',paddingLeft:2}}>{errMsg}</div>}
              <button onClick={handleSubmit} disabled={submitStatus==='loading'} className="btn btn-primary" style={{fontSize:15,padding:'13px',borderRadius:10,justifyContent:'center',width:'100%',opacity:submitStatus==='loading'?0.75:1}}>{submitStatus==='loading'?'Connecting you now…':'Get my AI receptionist →'}</button>
            </div>
      ...
      <section style={{background:"var(--bg-base)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)",padding:"32px 5%"}}><div style={{maxWidth:800,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,textAlign:"center"}}>{STATS.map(s=>(<div key={s.n}><div style={{fontSize:"clamp(28px,4vw,40px)",fontWeight:800,color:"var(--accent)",letterSpacing:"-.02em"}}>{s.n}</div><div style={{fontSize:13,color:"var(--t2)",marginTop:4,fontWeight:500}}>{s.l}</div></div>))}</div></section>
...
const LoginPage=({onLogin})=>{
  ...
  const handleLogin=async()=>{
    ...
    try{const {data,error:authErr}=await supabase.auth.signInWithPassword({email:email.trim(),password});if(authErr)throw authErr;const {data:profile}=await supabase.from('profiles').select('role').eq('id',data.user.id).single();onLogin(profile?.role||'client');}catch(err){setError(err.message||'Login failed. Check your email and password.');}finally{setLoading(false);}
  };
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg-base)",padding:24}}>
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:32}}><img src={logoFull} alt="AI Automated Calls" style={{height:52,width:"auto",objectFit:"contain",marginBottom:12}}/><p style={{fontSize:13,color:"var(--t3)",marginTop:4}}>aiautomatedcalls.com</p></div>
        <div className="card">
          ...
          <button className="btn btn-primary" onClick={handleLogin} disabled={loading} style={{width:"100%",justifyContent:"center",padding:"11px",fontSize:14,opacity:loading?0.75:1}}>{loading?<><Loader size={14} className="aac-spin"/>Signing in…</>:'Sign in'}</button>
          <div style={{textAlign:"center",marginTop:14}}><span style={{fontSize:12,color:"var(--t3)"}}>Forgot password? <span style={{color:"var(--accent)",cursor:"pointer"}} onClick={async()=>{if(email){await supabase.auth.resetPasswordForEmail(email);alert('Reset email sent!');}else{setError('Enter your email first.');}}}>Reset</span></span></div>
        </div>
...
export default function App(){
  ...
  useEffect(()=>{
    const resolve=async(session)=>{
      ...
      setRole(r);setPage(r==='admin'?'overview':'cdash');setScreen('app');
    };
    supabase.auth.getSession().then(({data:{session}})=>resolve(session));
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_,session)=>{if(!session){setScreen('marketing');setRole(null);}});
  ...
  if(screen==='loading') return(<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg-base)',gap:12,color:'var(--t3)',fontFamily:'var(--font)'}}><Loader size={20} className="aac-spin" color="var(--accent)"/><span style={{fontSize:14}}>Loading…</span></div>);
